...
function populateMonthGrid() {
  ...
  MONTHS.forEach((month, index) => {
    ...
    monthCard.addEventListener('click', () => {
      selectedMonth = index;
      hideMonthSelector();
      // Update month label and filter tabs
      updateMonthLabel();
      filterTabsByMonth();
    });
...
// Update month label tab
function updateMonthLabel() {
  const monthLabelText = document.getElementById('monthLabelText');
  if (monthLabelText && selectedMonth !== null) {
    monthLabelText.textContent = `${MONTHS[selectedMonth]} ${selectedYear}`;
  }
}
